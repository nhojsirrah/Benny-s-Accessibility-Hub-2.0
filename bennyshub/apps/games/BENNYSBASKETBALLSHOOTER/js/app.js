        ...
        class InputHandler {
            ...
            handleKeyUp(e) {
                ...
                // Menu selection accepts Enter AND NumpadEnter via the shared
                // ScanController.isSelect predicate (NumpadEnter parity). Gameplay
                // charging / shooting stays bound to Enter only (below), so the
                // app-specific shot timing is unchanged.
                const inMenu = (gameState === 'MENU' || gameState === 'PAUSED');
                const isSelectKey = MenuManager.menuScan
                    ? MenuManager.menuScan.isSelect(e)
                    : (e.code === 'Enter' || e.code === 'NumpadEnter');

                if (inMenu && isSelectKey) {
                    this.enterPressed = false;
                    clearTimeout(this.enterLongPressTimer); // Cancel pause timer
                    // If a long press triggered pause, do not select.
        ...
        class MenuSystem {
            constructor() {
                ...
                // movement + announce engine only. It is intentionally NOT
                // attached to the document: the same Space / Enter keys also
                // drive the app-specific aiming / charging timing during play
                // (see InputHandler), which stays on its own path. getTargets
                // exposes the active menu's live .menu-item elements; onFocus
                // reuses the existing highlight (activeIndex + updateVisuals);
                // onAnnounce reuses the existing beep + speakSelection; onSelect
                // runs the focused item's action. activeIndex stays the single
                // source of truth the visuals read; syncScan() aligns the
                // controller cursor to it before each move so state transitions
                ...
                this.menuScan = (typeof window !== 'undefined' && window.ScanController)
                    ? new window.ScanController({
                        ...
                        autoScan: false, // auto-scan stays on the app's own startAutoScan timer
                    })
                    : null;
            }

            // Align the shared ScanController cursor to the DOM-driven
            // activeIndex before a move (no-op without the shared controller).
            syncScan() {
                if (this.menuScan) this.menuScan.setIndex(this.activeIndex);
            }
            ...
            scanNext(isAuto = false) {
                ...
                if (this.menuScan) {
                    // Preserve the original manual "extra" beep; onAnnounce fires
                    // the always-beep + speak for every focus change.
                    if (!isAuto) audio.playBeep(200);
                    this.syncScan();
                    this.menuScan.advance();
                    return;
                }
                // Fallback (no shared ScanController): original behavior.
            ...
            selectCurrent() {
                ...
                if (this.menuScan) {
                    this.syncScan();
                    this.menuScan.select(); // -> onSelect -> handleAction
                } else {
                    // Fallback (no shared ScanController): original behavior.
                    const items = this.currentMenu.getElementsByClassName('menu-item');
            ...
            handleAction(action, element) {
                switch(action) {
                    ...
                    case 'exit':
                        ...
                        setTimeout(() => {
                            // Prefer the shared Nav back-contract; fall back to the
                            // legacy postMessage / location when Nav is absent.
       ...
       function applyVisualSettings() {
        ...
        // Wait for load. Skipped under CommonJS/test (module.exports defaults to
        // a truthy {}), so the jsdom harness can require() this file headless
        // without booting the Three.js render loop, audio, or the welcome speech.
        if (typeof module === 'undefined' || !module.exports) {
            window.addEventListener('load', init);
        }

        // CommonJS export surface for jsdom tests (no-op in the browser). Exposes
        // the menu / scan internals so tests can drive MENU / SETTINGS scanning
        // through the shared ScanController without booting the 3D game.
        if (typeof module !== 'undefined' && module.exports) {
            module.exports = {
                MenuManager,
                Input,
                ...
                getState: () => ({
                    gameState,
                    activeIndex: MenuManager.activeIndex,
                    tts: GameSettings.tts,
