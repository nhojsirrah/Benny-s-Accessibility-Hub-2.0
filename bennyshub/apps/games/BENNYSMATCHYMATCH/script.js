  ...
  setup: {
    ...
    difficulties: [
      ...
      { id: "3000", label: "Hard (3s)", tts: "Hard 3 Second Reveal" },
      {
        id: "1000",
        label: "Super Hard (1s)",
        tts: "Super Hard 1 Second Reveal",
      },
    ],
  },
  challengePlus: {
...
// --- Shared contracts (IP-5 adoption) ---------------------------------------
// Resolve shared modules from the global scope (browser <script src>) with a
// CommonJS fallback so this file is require()-able under jest + jsdom.
const Themes =
  (typeof window !== "undefined" && window.Themes) ||
  (typeof require === "function" ? require("../../../shared/themes.js") : null);
const SettingsStore =
  (typeof window !== "undefined" && window.SettingsStore) ||
  (typeof require === "function"
    ? require("../../../shared/settings-store.js")
    : null);
const GridMemoryPack =
  (typeof window !== "undefined" && window.GridMemoryPack) ||
  (typeof require === "function"
...
// Canonical highlight palette (shared/themes.js), adopted in place of the old
// local copy. Identical values, single source of truth.
const highlightColors = Themes.HIGHLIGHT_COLORS;

// Canonical background themes (shared/themes.js). MatchyMatch additionally needs
// a per-theme card-back color, which is NOT part of the shared theme contract,
// so we overlay it here. The bg gradients + highlights are byte-identical to the
// old local array (the old 'yellow'/'white' highlights are the canonical
// '#ffff00'/'#ffffff', which render the same), so this is behavior-preserving.
const MATCHY_CARD_BACKS = [
  "#444",
  "#003366",
  "#222",
  "#2e8b57",
...
const themes = Themes.THEMES.map((t, i) => ({
  ...t,
  cardBack: MATCHY_CARD_BACKS[i],
}));
...
// --- Persistence ---
// Adopted SettingsStore (shared/settings-store.js): per-app keys live in the
// 'matchy' app store, shared keys (highlight style/color) in the global store.
// The shared store's `matchy_settings` migration imports the old localStorage
// blob on first read, so existing players keep their settings. A localStorage
// fallback keeps the game working if the shared store is ever absent.
const MATCHY_APP_KEYS = [
  "themeIndex",
  "tts",
  "ttsLocation",
  "sound",
...
const MATCHY_GLOBAL_KEYS = ["highlightStyle", "highlightColorIndex"];

function saveSettings() {
  if (SettingsStore) {
    const appStore = SettingsStore.app("matchy");
    MATCHY_APP_KEYS.forEach((k) => appStore.set(k, settings[k]));
    MATCHY_GLOBAL_KEYS.forEach((k) => SettingsStore.global.set(k, settings[k]));
    return;
  }
  try {
...
function loadSettings() {
  if (SettingsStore) {
    SettingsStore.runMigrations(); // import any legacy matchy_settings blob
    const appStore = SettingsStore.app("matchy");
    MATCHY_APP_KEYS.forEach((k) => {
      const v = appStore.get(k);
      if (v !== undefined) settings[k] = v;
    });
    MATCHY_GLOBAL_KEYS.forEach((k) => {
      const v = SettingsStore.global.get(k);
      if (v !== undefined) settings[k] = v;
    });
    return;
  }
...
async function init() {
  ...
  try {
    const localReg = JSON.parse(
      localStorage.getItem("matchy_local_registry") || "[]",
    );
    ...
    localReg.forEach((p) => {
      ...
      if (existingIndex !== -1) {
        ...
        console.log(
          `Duplicate pack detected: ${state.packs[existingIndex]} vs ${p}. Preferring local.`,
        );
      ...
      if (data && data.categories) {
        ...
        for (const [catName, cards] of Object.entries(data.categories)) {
          if (state.assets.categories[catName]) {
            state.assets.categories[catName] =
              state.assets.categories[catName].concat(cards);
          } else {
...
function getPackTitle() {
  if (state.packs.length === 0 || !state.packs[state.currentPackIndex])
    return "Default";
...
async function cyclePack(dir) {
  ...
  state.currentPackIndex =
    (state.currentPackIndex + dir + state.packs.length) % state.packs.length;
...
const menus = {
  ...
  settings: [
    ...
    {
      text: () => `Location TTS: ${settings.ttsLocation ? "On" : "Off"}`,
      action: () => toggleTTSLocation(),
      onPrev: () => toggleTTSLocation(),
    },
    {
    ...
    {
      text: () =>
        `P1 Color: ${settings.p1Color}${getColorSwatch(settings.p1Color)}`,
      action: () => cycleP1Color(1),
      onPrev: () => cycleP1Color(-1),
    },
    {
      text: () =>
        `P2 Color: ${settings.p2Color}${getColorSwatch(settings.p2Color)}`,
      action: () => cycleP2Color(1),
      onPrev: () => cycleP2Color(-1),
    },
    { text: "Open Editor", action: () => showEditorWarning() },
    {
      text: () => `Voice: ${getVoiceName()}`,
      action: () => cycleVoice(1),
      onPrev: () => cycleVoice(-1),
    },
    {
  ...
  pauseSettings: [
    ...
    {
      text: () => `Location TTS: ${settings.ttsLocation ? "On" : "Off"}`,
      action: () => toggleTTSLocation(true),
      onPrev: () => toggleTTSLocation(true),
    },
    {
      text: () => `Sound: ${settings.sound ? "On" : "Off"}`,
      action: () => toggleSound(true),
      onPrev: () => toggleSound(true),
    },
    {
      text: () =>
        `Highlight Style: ${settings.highlightStyle === "outline" ? "Outline" : "Full Cell"}`,
    ...
    {
      text: () => `Voice: ${getVoiceName()}`,
      action: () => cycleVoice(1, true),
      onPrev: () => cycleVoice(-1, true),
    },
    {
      text: () => {
        if (window.NarbeScanManager)
  ...
  if (isElectron && window.electronAPI.editor) {
    window.electronAPI.editor
      ...
      .then((result) => {
        if (result.success) {
          console.log(
            "[Editor] Opened matchymatch editor in Chrome:",
            result.url,
          );
        } else {
...
function promptLoadGame() {
  ...
  input.onchange = async (e) => {
    ...
    try {
      ...
      const validPack = jsonResults.find(
        (r) => r && r.json && r.json.categories,
      );
      ...
      if (validPack) {
        ...
        let packIndex = state.packs.findIndex(
...
function showSetupMenu() {
  ...
  if (state.setup.categoryIndex >= state.setup.categories.length)
    state.setup.categoryIndex = 0;
...
function updateBoardSizes() {
  ...
  if (availableCount >= 8)
    state.setup.boardSizes.push({ label: "4x4", id: "4x4", tts: "4 by 4" });
  if (availableCount >= 10)
    state.setup.boardSizes.push({ label: "4x5", id: "4x5", tts: "4 by 5" });
  if (availableCount >= 15)
    state.setup.boardSizes.push({ label: "5x6", id: "5x6", tts: "5 by 6" });
  if (availableCount >= 18)
    state.setup.boardSizes.push({ label: "6x6", id: "6x6", tts: "6 by 6" });

  if (state.setup.boardSizes.length === 0) {
    state.setup.boardSizes.push({
      label: "Not enough cards",
      id: null,
      tts: "Not enough cards",
    });
  }
...
function updateSetupMenu() {
  menus.setup = [
    {
      text: () =>
        `Category: ${state.setup.categories[state.setup.categoryIndex]}`,
  ...
  if (state.gameMode === "challenge") {
    menus.setup.push({
      text: () =>
        `Difficulty: ${state.setup.difficulties[state.setup.difficultyIndex].label}`,
      tts: () =>
        `Difficulty: ${state.setup.difficulties[state.setup.difficultyIndex].tts}`,
...
function cycleCategory(dir) {
  state.setup.categoryIndex =
    (state.setup.categoryIndex + dir + state.setup.categories.length) %
    state.setup.categories.length;
  state.category = state.setup.categories[state.setup.categoryIndex];
  updateBoardSizes();
...
function cycleBoardSize(dir) {
  ...
  state.setup.boardSizeIndex =
    (state.setup.boardSizeIndex + dir + state.setup.boardSizes.length) %
    state.setup.boardSizes.length;
  renderMenu();
}

function cycleChallengeDifficulty(dir) {
  state.setup.difficultyIndex =
    (state.setup.difficultyIndex + dir + state.setup.difficulties.length) %
    state.setup.difficulties.length;
  renderMenu();
}
...
function showLevelAnnouncement(num, size, time) {
  ...
  container.innerHTML = `
    ...
    `;
  speak(
    `Level ${num}, ${size.replace("x", " by ")}, ${time / 1000} second reveal`,
  );
}
...
function cycleVoice(dir = 1, isPause = false) {
  if (window.NarbeVoiceManager) {
    ...
    if (voices.length > 0) {
      settings.voiceIndex =
        (settings.voiceIndex + dir + voices.length) % voices.length;
      saveSettings();
      speak("Voice changed");
    }
  }
...
function getVoiceName() {
  ...
  if (voices.length > 0) {
    return (
      voices[settings.voiceIndex % voices.length].name.slice(0, 15) + "..."
    );
  }
  return "Default";
}
...
function renderMenu() {
  ...
  if (state.menuState === "loadWarning") {
    ...
    info.innerText =
      "Upload your assets and/or packs folder to this browser and play your own custom game";
...
function renderPauseMenu() {
  ...
  title.innerText =
    state.pauseMenuState === "settings" ? "Pause Settings" : "Pause Menu";
  ...
  if (state.pauseIndex !== -1) {
    const txt =
      typeof items[state.pauseIndex].text === "function"
        ? items[state.pauseIndex].text()
        : items[state.pauseIndex].text;
    speak(txt.replace(/<[^>]*>/g, ""));
  }
}
...
function startGame(difficulty) {
  ...
  if (state.gameMode === "challenge" || state.gameMode === "challenge-plus") {
    ...
    state.mismatchLimit =
      limits[difficulty] !== undefined ? limits[difficulty] : 4;
    ...
    if (state.gameMode === "challenge") {
      if (
        state.setup &&
        state.setup.difficulties &&
        state.setup.difficulties[state.setup.difficultyIndex]
      ) {
...
function generateGrid() {
  ...
  // Gather the content cards for the selected category (same selection as before).
  let availableCards = [];
  const cats = state.assets.categories || {};
  if (state.category === "All") {
    Object.keys(cats).forEach((k) => {
      const cards = cats[k];
      if (Array.isArray(cards)) cards.forEach((c) => availableCards.push(c));
    });
  } else {
    const cards = cats[state.category];
    if (Array.isArray(cards)) availableCards = [...cards];
  }

  // Empty category: nothing to lay out (the engine requires a non-empty pack).
  if (availableCards.length === 0) {
    state.grid = [];
    return;
  }

  // Build a grid-memory pack from the live config + content cards, then let the
  // shared engine lay it out. A MatchyMatch board is now "engine + a pack": the
  // board dims, scan order, timing and tiles are all data. The engine's layout
  // builder reproduces the previous deterministic-by-rng arrangement exactly
  // (same Fisher-Yates over the same Math.random source), so this is
  // behavior-preserving. Each tile keeps its source card for sound/TTS lookup.
  const tiles = availableCards.map((card) => ({
    id: card.title || "Untitled",
  ...
  const pack = buildMatchyPack(state.rows, state.cols, tiles);
  const layout = GridMemoryPack.buildLayout(pack, { rng: Math.random });

  state.grid = layout.grid.map((cell) => {
    if (cell.inactive) return { r: cell.r, c: cell.c, inactive: true };
    const w = cell.source;
    state.soundMap[w.id] = w.card; // keyed by name, as before
    return {
      r: cell.r,
      c: cell.c,
      name: w.label,
...
/**
 * Assemble the grid-memory engine pack for a MatchyMatch round. This is the
 * "Matchy JSON pack" the engine consumes — board + scanOrder + timing +
 * difficulty + tiles. See matchy.pack.json in this game's folder for the
 * documented static exemplar of this same shape.
 */
function buildMatchyPack(rows, cols, tiles) {
  return {
    id: "matchy-runtime",
    title: getPackTitle ? getPackTitle() : "MatchyMatch",
    board: { rows, cols },
    scanOrder: "row-col",
    difficulty: state.difficulty,
    timing: { mismatchHideMs: 3000 },
    tiles,
  };
}

// --- Sound & UI Helpers ---
// Guarded so the module is require()-able under jsdom (no Web Audio API there).
const AudioCtxClass =
...
function playSystemSound(type) {
  ...
  if (!audioCtx) return; // no audio backend (e.g. tests)
    ...
    if (!cell.inactive) {
      ...
      if (cell.matched) {
        btn.style.backgroundColor =
          cell.owner === 1 ? settings.p1Color : settings.p2Color;
        ...
        img.alt =
          cell.altTitle && cell.altTitle.trim() !== ""
            ? cell.altTitle
            : cell.name;
  ...
  if (state.scan.mode === "row") {
    ...
    if (cell && !cell.matched && !cell.revealed) {
      const btn = document.getElementById(
        `btn-${state.scan.row}-${state.scan.col}`,
      );
      if (btn) {
        if (settings.highlightStyle === "outline") {
          btn.style.border = `8px solid ${t.highlight}`;
...
function moveGameScan(dir) {
  ...
  if (state.scan.mode === "row") {
    ...
    if (settings.ttsLocation && state.scan.row !== -1)
      speak(`Row ${state.scan.row + 1}`);
  } else {
...
function rowHasUnmatched(r) {
  return state.grid.some(
    (cell) => cell.r === r && !cell.matched && !cell.inactive && !cell.revealed,
  );
}
...
function selectGameOption() {
  ...
  if (state.scan.mode === "row") {
    state.scan.mode = "col";
    const firstCol = state.grid.find(
      (cell) =>
        cell.r === state.scan.row &&
        !cell.matched &&
        !cell.inactive &&
        !cell.revealed,
    );
...
function handleRoundEnd(winner) {
  ...
  if (winner !== 0 && state.scores[winner] >= 10) {
    // Bumped win score slightly or keep 5? Keep logic simple, user didn't ask to change win condition.
    showMatchWinner(winner);
  } else {
    // Progression
    let nextDiff = "4x4";
    if (state.difficulty === "4x4" || state.difficulty === "easy")
      nextDiff = "4x5";
    else if (state.difficulty === "4x5" || state.difficulty === "medium")
      nextDiff = "4x4"; // Cycle back for now to be safe?
...
function announceRoundStart() {
  ...
  speak(
    `${roundName} round, ${state.difficulty} mode, the score is ${state.scores[1]} to ${state.scores[2]}.`,
  );
}
...
// Start. Use window.onload (not a bare init() call) so require()ing this module
// under jest + jsdom does NOT auto-boot the game — the conformance test drives
// the exported internals directly. In the browser onload fires as usual.
if (typeof window !== "undefined") {
  window.onload = init;
}

// Dual export so the engine-backed board + scan + select layer can be exercised
// under jest + jsdom, mirroring the shared modules + TicTacToe. No-op in the
